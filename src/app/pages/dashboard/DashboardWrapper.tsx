...
const DashboardPage: FC = () => {
  ...
  const [hebrewButtons, setHebrewButtons] = useState({
    ButtonAbsenceAllDay: '',
    ButtonAbsenceEnd: '',
    ButtonAbsenceStart: '',
    ButtonAttendanceEnd: '',
    ButtonAttendanceStart: '',
    ButtonSickAllDay: '',
    ButtonSickEnd: '',
    ButtonSickStart: '',
  })
  const [buttonActions, setButtonActions] = useState({
    allowAbsenceAllDay: false,
    allowAbsenceEnd: false,
    allowAbsenceStart: false,
    allowAttendanceEnd: false,
    allowAttendanceStart: false,
    allowSickAllDay: false,
    allowSickEnd: false,
    allowSickStart: false,
  })
        ...
        <div>
          ...
          <div className='button-parent-div'>
            ...
            <button
              ...
              className='btn btn-lg btn-secondary w-170-px mb-5 ml-xl-100'
            >
              {activeRow === 'sick' && latestStartTime !== null
                ? latestStartTime
  ...
  const constructAttandancePageContent = () => {
    return (
      <>
        <div>
          {workActivityCodeItems && workActivityCodeItems.length && (
            <select
              aria-label=''
          ...
          <div className='button-parent-div'>
            <button
              ...
              className='btn btn-lg btn-secondary w-170-px mb-5 ml-xl-100'
            >
              {activeRow === 'attendance' && latestStartTime !== null
                ? latestStartTime
  ...
  const constructAbsencePageContent = () => {
    return (
      <>
        <div>
          {absenceTypeItems && absenceTypeItems.length && (
            <select
              aria-label=''
              data-control='select2'
              data-placeholder='Work Activity Code'
          ...
          <div className='button-parent-div'>
            ...
            <button
              ...
              className='btn btn-lg btn-secondary w-170-px mb-5 ml-xl-100'
            >
              {activeRow === 'absence' && latestStartTime !== null
                ? latestStartTime
    ...
    <>
      <div style={{height: 'auto'}} className='main-container-dashboard'>
        <ul
          ...
          className='nav nav-tabs nav-tabs-line mb-7'
        >
          {tabs.length &&
            tabs.map(({name, id, label}) => {
              // ${name.charAt(0).toUpperCase()+name.slice(1)}
              return (
                <li key={id} className='nav-item' onClick={handleChangeTab(name)}>
                  <span
                    className={`nav-link tab ${getSelectedClass(name)} `}
                    data-toggle='tab'
                  >{`${label}`}</span>
                </li>
              )
